...
class ForgotPassword extends Component {
  ...
  handleSubmitForgotPassword = async (e, type) => {
    ...
    if (type === false) {
      ...
      let res = await this.props.retrievalPassword({
        ...this.state.form,
      ...
      if (
        (res && res.data && res.data.errCode === 1) ||
        res.data.errCode === 0
      ) {
        this.setState({
          isModal: false,
  ...
  render() {
    ...
    return (
      <>
        ...
        {!isSuccess ? (
          <form className={cx("forgotpassword")}>
            <span className={cx("heading")}>
              <SwitchLanguage id="app.editPass.heading" />
            </span>
            <div className={cx("form-input")}>
              {!isFormCreateNewPassword && (
                <>
                  <label>
                    <SwitchLanguage id="app.editPass.labelEmail" />
                  </label>
                  <input
                    type="text"
                    name="valueAccount"
                  ...
                  <span className={cx("error-input-login")}>
                    ...
                    {!_.isEmpty(erroSearchEmail)
                      ? language === languages.EN
                        ? `${erroSearchEmail.account.valueEn}`
                        : `${erroSearchEmail.account.valueVi}`
                      : ""}
                  </span>
                </>
              )}
              ...
              {isFormCreateNewPassword && (
                <>
                  <label>
                    <SwitchLanguage id="app.editPass.labelNewPass" />
                  </label>
                  <input
                    type="text"
                    name="newPassword"
                    ...
                    autoComplete="off"
                    style={{
                      boxShadow: isNewPassword ? "1px 1px 10px #046ba6" : "",
                    }}
                    onChange={(e) =>
                      this.handleChangeInputAccount(
                        e.target.value,
                    ...
                    onFocus={() => this.setState({ isNewPassword: true })}
                    placeholder={
                      language === languages.EN
                        ? "Enter new password"
                        : "Nhập mật khẩu mới"
                    }
                    onBlur={() => this.setState({ isNewPassword: false })}
                  />

                  <label>
                    <SwitchLanguage id="app.editPass.labelRetype" />
                  </label>
                  <input
                    type="text"
                    name="retypePassword"
                  ...
                  <label>
                    <SwitchLanguage id="app.editPass.labelCode" />
                  </label>
                  <input
                    type="text"
                    name="token"
                    ...
                    className={cx("input")}
                    autoComplete="off"
                    style={{ boxShadow: isToken ? "1px 1px 10px #046ba6" : "" }}
                    placeholder={
                      language === languages.EN
                    ...
                    onFocus={() =>
                      this.setState({ isToken: true, erroToken: {} })
                    }
                    onChange={(e) =>
                      this.handleChangeInputAccount(
                        e.target.value,
                    ...
                    onBlur={() => this.setState({ isToken: false })}
                  />
                  <span className={cx("error-input-login")}>
                    {!_.isEmpty(erroToken) ? (
                      <FontAwesomeIcon icon={faCircleExclamation} />
          ...
          <div className={cx("notification")}>
            <p>
              <SwitchLanguage id="app.editPass.ntfSuccess" />
            </p>
            <span>
              <Link to={path.LOGINPAGE}>
                <SwitchLanguage id="app.editPass.goLogin" />
              </Link>
            </span>
          </div>
        )}
...
const mapDispatchToProps = (dispatch) => {
  return {
    retrievalPassword: (email) =>
      dispatch(actions.retrievalPasswordStart(email)),
