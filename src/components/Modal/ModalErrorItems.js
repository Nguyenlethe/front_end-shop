...
class ModalErrorItems extends Component {
  ...
  handleShowHideModal = (e, actions) => {
    ...
    if (
      ...
      e.target.className === "content-btn-form-input close"
    ) {
      this.setState({
        isShowModal: false,
      });
  ...
  render() {
    ...
    return (
      <>
        <div
          className="modal"
          onClick={(e) => this.handleShowHideModal(e)}
          style={{
            height: isShowModal ? "100vh" : "0vh",
            opacity: isShowModal ? "1" : "0",
            zIndex: isShowModal ? "9999999" : "0",
          }}
          id="Modal-warn"
        >
          {title === "warn" && (
            <div className="col l-5 form-notification">
              {title === "warn" && (
                <div className="title">
                  <p>
                    {" "}
                    <SwitchLanguage id="manageAdmin.modal.warn" />{" "}
                  </p>

                  <span
                    className="close"
                    onClick={(e) => this.handleShowHideModal(e, "ICON")}
                  >
