      ...
      <div className="nav-top-right">
       {!islogin ? (
        ...
        <div className="list-icon-nav">
         ...
         {islogin == "true" ? (
          <div
           className="pile-profile"
           onMouseLeave={() => this.handleSetShowOrHideListCart("HIDE")}
          >
           <div
            className="wrapper_avatar mgl-12"
            onMouseOver={() => this.handleSetShowOrHideListCart("SHOW")}
            onClick={() => this.props.navigate(path.PRO_FILE)}
           >
            <img
             src={`${process.env.REACT_APP_BACKEND_IMAGES_URL}/${avatar}`}
             alt=""
            />
           </div>
...
const mapDispatchToProps = (dispatch) => {
 return {
  getLikeOrFollowItemsShop: (data) =>
   dispatch(action.getLikeOrFollowItemsShop(data)),
 };
};
