...
class Items extends Component {
  ...
  heandleDataForm = (action) => {
    let { allShops, allcategory, DCCData, BNPRDData, TYPESIZEData, COLORData } =
      this.props;
    ...
    let newListCategory = generalHandling.handlConvertObject(
      allcategory,
      "LIST_CATEGORY",
      this.props.language
    );
    let newDCCData = generalHandling.handlConvertObject(
      DCCData,
      "LIST_CATEGORY",
      this.props.language
    );
    let newBNPRDData = generalHandling.handlConvertObject(
      BNPRDData,
      "LIST_CATEGORY",
      this.props.language
    );
    let newTYPESIZEData = generalHandling.handlConvertObject(
      TYPESIZEData,
      "LIST_CATEGORY",
      this.props.language
    );
    let newCOLORData = generalHandling.handlConvertObject(
      COLORData,
      "LIST_CATEGORY",
      this.props.language
    );
  ...
  componentDidUpdate = async (prevProps, prevState) => {
    let { FSBData, FSAMData, FSMData, FSAWData, AllData, SIZEData, SZNBData } =
      this.props;
    ...
    if (prevProps.language !== this.props.language) {
      ...
      if (optionsCategory) {
        ...
        var newListAllCategoryType = generalHandling.handlConvertObject(
          allTypeCategory,
          "LIST_CATEGORY",
          this.props.language
        );
      ...
      let newListColor = generalHandling.handlConvertObject(
        COLORData,
        "LIST_CATEGORY",
        this.props.language
      );
      ...
      let newlistItemsSizeAmount = generalHandling.handlConvertObject(
        TYPESIZEData,
        "LIST_CATEGORY",
        this.props.language
      );
      ...
      listImg.map((item) => {
        newListColor.map((color) => {
          let colorOptionImg =
            item.file && item.file.option && item.file.option.value;
    ...
    if (prevProps.FSBData !== this.props.FSBData) {
      let allFSBData = generalHandling.handlConvertObject(
        FSBData,
        "LIST_CATEGORY",
        this.props.language
      );
    ...
    if (prevProps.FSAMData !== this.props.FSAMData) {
      let allFSAMData = generalHandling.handlConvertObject(
        FSAMData,
        "LIST_CATEGORY",
        this.props.language
      );
    ...
    if (prevProps.FSMData !== this.props.FSMData) {
      let allFSMData = generalHandling.handlConvertObject(
        FSMData,
        "LIST_CATEGORY",
        this.props.language
      );
    ...
    if (prevProps.FSAWData !== this.props.FSAWData) {
      let allFSAWData = generalHandling.handlConvertObject(
        FSAWData,
        "LIST_CATEGORY",
        this.props.language
      );
    ...
    if (prevProps.AllData !== this.props.AllData) {
      let allAllData = generalHandling.handlConvertObject(
        AllData,
        "LIST_CATEGORY",
        this.props.language
      );
    ...
    if (res && res.data.errCode === 0) {
      ...
      data.label =
        language === languages.VI
          ? res.data.data.labelVi
          : res.data.data.labelEn;
    }
    return data;
  };
  ...
  handlChangeSlelect = async (valueOptions, name) => {
    ...
    if (
      ...
      listImg.map((img) => {
        ...
        stateCoppyitemsColorImgages.push({
          color: img.file.option.value,
          itemId: idItems,
        });
      });
      ...
      listImg.map((item) => {
        if (
          item.file.option === "" ||
          item.file.option === null ||
          item.file.option === undefined
        ) {
          isImgColor = false;
        }
      });
  ...
  heandleChangeInputSize = (value, item) => {
    let { SIZE, SZNB, notSize, itemsSizeAmount, listErrorForm, listSIZEData } =
      this.state;
    let { idItems } = this.state.items;
  ...
  handleOnSubmit = async (e) => {
    ...
    if (editItems) {
      ...
      listImgFormData.length > 0 &&
        listImgFormData.map((item) => {
          if (!item.type) {
            dataImgColor.push({
              itemId: items.idItems,
              color: item.option.value,
              image: item.id,
            });
          } else {
            let nameImg = `name${new Date().getTime()}${Math.floor(
      ...
      let dataItemsInfo = JSON.stringify({
        ...itemsInfo,
    ...
    if (!editItems) {
      ...
      for (let key in itemsColorImgages) {
        if (listImgFormData[key]) {
          ...
          itemsColorImgages[
            key
          ].image = `name${new Date().getTime()}${Math.floor(
            1000 + Math.random() * 9000
          )}.jpg`;
          data.append(
            "file",
            listImgFormDataArray[key],
            `${itemsColorImgages[key].image}`
          );
        }
      }
  ...
  handleEditItems = async (dataItems) => {
    let { items, editItems, itemsInfo } = this.state;
    let { language } = this.props;
    ...
    if (editItems) {
      ...
      let nameShop = {
        value: dataItems.idShop,
        label: dataItems.storeData && dataItems.storeData.nameShop,
      };
      let sentFromEdit =
        dataItems.infoItemsData && dataItems.infoItemsData.sentFrom;
      let idItemsEdit = dataItems.idItems;

      let optionsCategoryEdit = {
        value: dataItems.categoryData && dataItems.categoryData.code,
        label:
          dataItems.categoryData && language === languages.EN
            ? dataItems.categoryData.valueEn
            : dataItems.categoryData.valueVi,
      };

      await this.props.getAllCodeInToItems(optionsCategoryEdit.value);

      let optionsCategoryTypeEdit = {
        value: dataItems.typeData && dataItems.typeData.code,
        label:
          dataItems.typeData && language === languages.EN
            ? dataItems.typeData.valueEn
            : dataItems.typeData.valueVi,
      };

      let optionsSeleEdit = {
        value: dataItems.discountData && dataItems.discountData.code,
        label:
          dataItems.discountData && language === languages.EN
            ? dataItems.discountData.valueEn
            : dataItems.discountData.valueVi,
      };

      let nameEdit = dataItems.name;
      let nameEnEdit = dataItems.nameEn;

      let optionsTrademarkEdit = {
        value:
          dataItems.infoItemsData &&
          dataItems.infoItemsData.trademarkData &&
          dataItems.infoItemsData.trademarkData.code,
        label:
          dataItems.infoItemsData.trademarkData && language === languages.EN
            ? dataItems.infoItemsData.trademarkData.valueEn
            : dataItems.infoItemsData.trademarkData.valueVi,
      };

      let productionEdit =
        dataItems.infoItemsData && dataItems.infoItemsData.production;
      let textureEdit =
        dataItems.infoItemsData && dataItems.infoItemsData.texture;

      let optionsItemsSizeAmountEdit = {
        value:
          dataItems.dataSizeAmount &&
          dataItems.dataSizeAmount[0].typeSizeData &&
          dataItems.dataSizeAmount[0].typeSizeData.code,
        label:
          dataItems.dataSizeAmount[0].typeSizeData && language === languages.EN
            ? dataItems.dataSizeAmount[0].typeSizeData.valueEn
            : dataItems.dataSizeAmount[0].typeSizeData.valueVi,
      };
      ...
      //
      let describeTextEnEdit =
        dataItems.infoItemsData && dataItems.infoItemsData.describeTextEn;
      let describeTextViEdit =
        dataItems.infoItemsData && dataItems.infoItemsData.describeTextVi;
      let describeHtmlEnEdit =
        dataItems.infoItemsData && dataItems.infoItemsData.describeHtmlEn;
      let describeHtmlViEdit =
        dataItems.infoItemsData && dataItems.infoItemsData.describeTextVi;
      ...
      dataItems.dataColorImg.map((item) => {
        listImgColor.push({
          ...
          file: {
            option: {
              value: item.colorData.code,
              label:
                language === languages.EN
                  ? item.colorData.valueEn
                  : item.colorData.valueVi,
            },
            id: item.image,
          },
        });
      ...
      <>
        ...
        >
          {!isShowListsInput && (
            <Button
              ...
              content={<SwitchLanguage id="manageAdmin.createItems" />}
              icon={<FontAwesomeIcon className="icon-user" icon={faStore} />}
        ...
        <div className="l-12">
          <p className="heading-manage-user">
            <SwitchLanguage id="manageAdmin.listItem" />
          </p>
        </div>
        ...
        {isShowListsInput && (
          <div
            ...
            className="all-input items l-12"
          >
            <div className="list-input">
              <div className="form-input col l-6">
                ...
                <span className="err">
                  {" "}
                  {!_.isEmpty(listErrorForm.idShop) && (
                    <FontAwesomeIcon icon={faCircleExclamation} />
                  )}
                  {!_.isEmpty(listErrorForm.idShop)
                    ? language === languages.VI
                      ? listErrorForm.idShop.valueVi
                      : listErrorForm.idShop.valueEn
                    : ""}
                </span>
              </div>
              ...
              <div className="form-input col l-6">
                <label className="input-label">
                  <SwitchLanguage id="manageAdmin.form.manageId" />
                </label>
                <Select
                  isDisabled={true}
            ...
            <div className="list-input">
              <div className="form-input col l-6">
                <label className="input-label">
                  <SwitchLanguage id="manageAdmin.form.idItems" />
                </label>
                <input
                  type="text"
                  className="input"
                  ...
                  disabled={editItems ? true : false}
                />
                <span className="planceholder_input">
                  <SwitchLanguage id="manageAdmin.form.planceholder_idItems" />
                </span>
                <span className="err ">
                  {" "}
                  {!_.isEmpty(listErrorForm.idItems) && (
                    <FontAwesomeIcon icon={faCircleExclamation} />
                  )}
                  {!_.isEmpty(listErrorForm.idItems)
                    ? language === languages.VI
                      ? listErrorForm.idItems.valueVi
                      : listErrorForm.idItems.valueEn
                    : ""}
                </span>
              </div>
              ...
              <div className="form-input col l-6">
                ...
                <span className="err">
                  {" "}
                  {!_.isEmpty(listErrorForm.category) && (
                    <FontAwesomeIcon icon={faCircleExclamation} />
                  )}
                  {!_.isEmpty(listErrorForm.category)
                    ? language === languages.VI
                      ? listErrorForm.category.valueVi
                      : listErrorForm.category.valueEn
                    : ""}
                </span>
              </div>
            </div>
            ...
            <div className="list-input">
              <div className="form-input col l-6">
                ...
                <span className="err">
                  {" "}
                  {!_.isEmpty(listErrorForm.type) && (
                    <FontAwesomeIcon icon={faCircleExclamation} />
                  )}
                  {!_.isEmpty(listErrorForm.type)
                    ? language === languages.VI
                      ? listErrorForm.type.valueVi
                      : listErrorForm.type.valueEn
                    : ""}
                </span>
              </div>
            ...
            <div className="list-input">
              <div className="form-input col l-6">
                <label className="input-label">
                  <SwitchLanguage id="manageAdmin.form.nameItems" />
                </label>
                <input
                  type="text"
                  className="input"
                ...
                <span className="err">
                  {" "}
                  {!_.isEmpty(listErrorForm.name) && (
                    <FontAwesomeIcon icon={faCircleExclamation} />
                  )}
                  {!_.isEmpty(listErrorForm.name)
                    ? language === languages.VI
                      ? listErrorForm.name.valueVi
                      : listErrorForm.name.valueEn
                    : ""}
                </span>
              </div>
              ...
              <div className="form-input col l-6">
                <label className="input-label">
                  <SwitchLanguage id="manageAdmin.form.nameItemsEn" />
                </label>
                <input
                  type="text"
                  className="input"
                ...
                <span className="err">
                  {" "}
                  {!_.isEmpty(listErrorForm.nameEn) && (
                    <FontAwesomeIcon icon={faCircleExclamation} />
                  )}
                  {!_.isEmpty(listErrorForm.nameEn)
                    ? language === languages.VI
                      ? listErrorForm.nameEn.valueVi
                      : listErrorForm.nameEn.valueEn
                    : ""}
                </span>
              </div>
            </div>
            ...
            <div className="list-input">
              <div className="form-input col l-6">
                ...
                <Select
                  ...
                  name="trademark"
                  placeholder={
                    <SwitchLanguage id="manageAdmin.form.planceholder_trademark" />
                  }
                />
                <span className="err">
                  {" "}
                  {!_.isEmpty(listErrorForm.trademark) && (
                    <FontAwesomeIcon icon={faCircleExclamation} />
                  )}
                  {!_.isEmpty(listErrorForm.trademark)
                    ? language === languages.VI
                      ? listErrorForm.trademark.valueVi
                      : listErrorForm.trademark.valueEn
                    : ""}
                </span>
              </div>
              ...
              <div className="form-input col l-6">
                ...
                />
                <span className="planceholder_input">
                  <SwitchLanguage id="manageAdmin.form.planceholder_production" />
                </span>
                <span className="err"></span>
              </div>
            </div>
            ...
            <div className="list-input">
              <div className="form-input col l-6">
                ...
                />
                <span className="planceholder_input">
                  <SwitchLanguage id="manageAdmin.form.planceholder_sentFrom" />
                </span>
                <span className="err">
                  {" "}
                  {!_.isEmpty(listErrorForm.sentFrom) && sentFrom === "" && (
                    <FontAwesomeIcon icon={faCircleExclamation} />
                  )}
                  {!_.isEmpty(listErrorForm.sentFrom) && sentFrom === ""
                    ? language === languages.VI
                      ? listErrorForm.sentFrom.valueVi
                      : listErrorForm.sentFrom.valueEn
                    : ""}
                </span>
              </div>
              ...
              <div className="form-input col l-6">
                ...
                <span className="planceholder_input">
                  <SwitchLanguage id="manageAdmin.form.planceholder_texture" />
                </span>
                <span className="err"></span>
              </div>
            </div>
            ...
            <div className="list-input">
              <div className="form-input col l-6">
                <label className="input-label">
                  <SwitchLanguage id="manageAdmin.form.price" />
                </label>
                <input
                  type="text"
                  className="input"
                ...
                />
                <span className="planceholder_input">
                  <SwitchLanguage id="manageAdmin.form.planceholder_price" />
                </span>
                <span className="err">
                  {" "}
                  {!_.isEmpty(listErrorForm.price) && (
                    <FontAwesomeIcon icon={faCircleExclamation} />
                  )}
                  {!_.isEmpty(listErrorForm.price)
                    ? language === languages.VI
                      ? listErrorForm.price.valueVi
                      : listErrorForm.price.valueEn
                    : ""}
                </span>
              </div>
              ...
              <div className="form-input col l-6">
                <label className="input-label">
                  <SwitchLanguage id="manageAdmin.items.priceUS" />
                </label>
                <input
                  type="text"
                  className="input"
                ...
                />
                <span className="planceholder_input">
                  <SwitchLanguage id="manageAdmin.items.planceholder_priceUS" />
                </span>
                <span className="err">
                  {" "}
                  {!_.isEmpty(listErrorForm.priceUS) && (
                    <FontAwesomeIcon icon={faCircleExclamation} />
                  )}
                  {!_.isEmpty(listErrorForm.priceUS)
                    ? language === languages.VI
                      ? listErrorForm.priceUS.valueVi
                      : listErrorForm.priceUS.valueEn
                    : ""}
                </span>
              </div>
            </div>
            ...
            <div className="list-input">
              <div className="form-input col l-6">
                <label className="input-label">
                  <SwitchLanguage id="manageAdmin.form.itemsSizeAmount" />
                </label>
                <Select
                  ...
                  placeholder={
                    <SwitchLanguage id="manageAdmin.form.planceholder_itemsSizeAmount" />
                  }
                />
                <span className="err"></span>
              </div>
            ...
            <div className="list-input">
              {optionsItemsSizeAmount &&
                optionsItemsSizeAmount.value === "FRSZ" && (
                  <div className="form-input col l-6">
                    <>
                      <label className="input-label">
                        <SwitchLanguage id="manageAdmin.form.amount" />
                      </label>
                      <input
                        type="number"
                        className="input"
                        onChange={(e) =>
                          this.heandleChangeInputSize(e.target.value, "FRSZ")
                        }
                        placeholder={
              ...
              {optionsItemsSizeAmount &&
                optionsItemsSizeAmount.value !== "FRSZ" && (
                  <div className="list-input size">
                    {listSIZEData &&
                      listSIZEData.length > 0 &&
                      listSIZEData.map((item, index) => {
                        return (
                          <div key={index} className="list-input-size col l-3">
                            <span className="input-label">
                              {languages.EN === language ? "Size" : "Cỡ"}{" "}
                              {item.valueEn}
                            </span>
                            <input
                              type="number"
              ...
              <span className="err">
                {" "}
                {!_.isEmpty(listErrorForm.itemsSizeAmount) && (
                  <FontAwesomeIcon icon={faCircleExclamation} />
                )}
                {!_.isEmpty(listErrorForm.itemsSizeAmount)
                  ? language === languages.VI
                    ? listErrorForm.itemsSizeAmount.valueVi
                    : listErrorForm.itemsSizeAmount.valueEn
                  : ""}
              </span>
            </div>
            ...
            <div className="list-input">
              <div className="col l-12">
                <label className="input-label">
                  <SwitchLanguage id="manageAdmin.form.description" />
                </label>

                <div className="list-btn" style={{ marginLeft: "-12px" }}>
                  <div
                    className="col l-2"
                    onClick={() => this.setState({ isBTNVi: !isBTNVi })}
                  >
                    <Button
                      type={
                        isBTNVi === true
                      ...
                      content={
                        <SwitchLanguage id="manageAdmin.form.descriptionVi" />
                      }
                      color={isBTNVi && "#ce163b"}
                      width="100%"
                      border="4px"
                    />
                  </div>
                  ...
                  >
                    <Button
                      ...
                      content={
                        <SwitchLanguage id="manageAdmin.form.descriptionEn" />
                      }
                      color={!isBTNVi && "#ce163b"}
                      width="100%"
                      border="4px"
                ...
                <MdEditor
                  ...
                  style={{
                    display: isBTNVi && isBTNVi ? "block" : "none",
                    height: "70vh",
                    overflow: "hidden",
                  }}
                ...
                <span className="err">
                  {" "}
                  {!_.isEmpty(listErrorForm.describeTextVi) && (
                    <FontAwesomeIcon icon={faCircleExclamation} />
                  )}
                  {!_.isEmpty(listErrorForm.describeTextVi)
                    ? language === languages.VI
                      ? listErrorForm.describeTextVi.valueVi
                      : listErrorForm.describeTextVi.valueEn
                    : ""}
                </span>
                <br></br>
                ...
                <span className="err">
                  {" "}
                  {!_.isEmpty(listErrorForm.describeTextEn) && (
                    <FontAwesomeIcon icon={faCircleExclamation} />
                  )}
                  {!_.isEmpty(listErrorForm.describeTextEn)
                    ? language === languages.VI
                      ? listErrorForm.describeTextEn.valueVi
                      : listErrorForm.describeTextEn.valueEn
                    : ""}
                </span>
              </div>
            </div>
            ...
            {idItems && (
              <div className="list-input img">
                {listImg &&
                  listImg.length > 0 &&
                  listImg.map((srcImg, index) => {
                    return (
                      <div key={`${srcImg.src}${index}`} className="col l-2-4">
                        <div
                          className="ct__product-show"
                          style={{
                        ...
                        >
                          <FontAwesomeIcon
                            ...
                            onClick={() => this.deleteImgPewView(srcImg)}
                          />
                        </div>
                        {/* color */}
                        <Select
                          ...
                          name={srcImg.file.id}
                          placeholder={
                            <SwitchLanguage id="manageAdmin.form.planceholder_color" />
                          }
                        />
                      </div>
                    );
                ...
                <div id="add" className="col l-2-4">
                  <div className="border-add-img">
                    ...
                    <span>
                      <SwitchLanguage id="manageAdmin.form.addImg" />
                    </span>
                  </div>
                  <input
                    type="file"
                    ...
                    multiple
                    onChange={(e) =>
                      this.heandleChangeInput(e, e.target.name, e)
                    }
                  />
                </div>
              </div>
            ...
            <span className="err">
              {" "}
              {!_.isEmpty(listErrorForm.file) && (
                <FontAwesomeIcon icon={faCircleExclamation} />
              )}
              {!_.isEmpty(listErrorForm.file)
                ? language === languages.VI
                  ? listErrorForm.file.valueVi
                  : listErrorForm.file.valueEn
                : ""}
            </span>
              ...
              <div className="list-btn">
                ...
                >
                  <Button
                    ...
                    content={
                      !editItems ? (
                        <SwitchLanguage id="manageAdmin.form.addItems" />
                      ) : (
                        <SwitchLanguage id="manageAdmin.form.btn_edit" />
                      )
                    }
                  />
                </span>
        ...
        {!isShowListsInput && (
          <ListItems
            handleEditItems={this.handleEditItems}
            setStateItems={this.setEditItems}
          />
        )}

        {listSale.length > 0 &&
          listAllShops.length > 0 &&
