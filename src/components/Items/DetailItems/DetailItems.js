...
class DetailItems extends Component {
    ...
    componentDidMount = async ()=>  {
        ...
        this.props.getDataItems({amount: 'ONE',idItems: idItems, idShop: 'EMPTY', category:  'EMPTY', type: 'EMPTY'})
    ...
    componentDidUpdate= async(prevProps, prevState)=> {
      ...
      if(prevProps.oneItems !== this.props.oneItems){
        ...
        if(this.props.oneItems[0] && this.props.oneItems[0].allStar && this.props.oneItems[0].allStar.medium){
        ...
        if(item){ provinceFreeShip = item.filter(ship => { if(ship.priceShipVN == 0){return (ship)}})}
        let itemCategory = oneItems[0].dataShipCategory
        if(itemCategory){ provinceFreeShipCategory = itemCategory.filter(ship => { if(ship.priceShipVN == 0){return (ship)}})} 
        let itemType = oneItems[0].dataShipCategoryType
        if(itemType){ provinceFreeShipType = itemType.filter(ship => { if(ship.priceShipVN == 0){return (ship)}})}
    ...
    handleAddLikeOrQuitLike =  async(idItems) => {
      ...
      if(isActive == true && isLike < 0){
        let res = await appService.getLikeOrFollowItemsShop({idUser: id,idShop: items.idShop, type: 'DELETE', idItems: idItems})
        // console.log('Bỏ like', res)
      }

      // Like
      if(isActive == false && isLike > 0){
        let res = await appService.getLikeOrFollowItemsShop({idUser: id, idShop: items.idShop, type: 'ADD', idItems: idItems})
    ...
    let {
      provinceFreeShipCategory,
      provinceFreeShipCategoryType,
      items,indexImg,isLike,countLike,
      isActive,isLoadItems,evaluate,
      countEvaluate,countSold,
      discountData,newPrice,
      newPriceUS,price, priceUS,
      provinceFreeShip,
      allDataPriceShip,
      priceItemsShip,
      priceShipType,
    ...
    <>
            ...
            <div className='col l-4'>
              <div className='border-img-items'> 
                  ...
                  <div className='border-img-show'>
                    {items.dataColorImg && items.dataColorImg[indexImg] && items.dataColorImg[indexImg].image ?
                      <img src={`${process.env.REACT_APP_BACKEND_IMAGES_ITEMS}/${items.dataColorImg[indexImg].image}`} alt='Ảnh sản phẩm '/>
                      :
                  ...
                  <div className='l-12 hug_list-img'>
                    <FontAwesomeIcon className='icon-next-left' icon={faAngleLeft} onClick={() => this.handleNextImg('BACK')} />

                    <div className='list-img-detail-items' ref={this.listImgItems}>
                      {items.dataColorImg && items.dataColorImg.map((img, index) => {
                        return (
                          <div className='col l-3' key={img.image} onClick={() => this.setState({indexImg: index})}>
                            <div className='wraper-img'>
                              <img src={`${process.env.REACT_APP_BACKEND_IMAGES_ITEMS}/${img.image}`} 
                                alt='Ảnh sản phẩm '  style={{border: index == indexImg ? '2px solid red' : '', }}
                              />
                            </div>
                          </div>
                        )
                    ...
                    <FontAwesomeIcon className='icon-next-right' icon={faAngleRight} onClick={() => this.handleNextImg('NEXT')} />
                  </div>

                  <div className='list-to-share l-6'>
                    {/* <span className='text-to-share'><SwitchLanguage  id='manageAdmin.items.toShare'/></span> */}

                    <a href='https://www.facebook.com/sharer/sharer.php?u=https://www.inithtml.com/' target="_blank" className='icon facebook'>
                      <FaFacebook />
                    </a>

                    <a href="https://www.facebook.com/sharer/sharer.php?u=https://www.inithtml.com/"  target="_blank"  className='icon message'>
                      <FaFacebookMessenger />  
                    </a>

                    <a  href={`https://twitter.com/share?text=Init%20HTML%20%E2%80%93%20Kh%E1%BB%9Fi%20%C4%91%E1%BA%A7u%20d%E1%BB%B1%20%C3%A1n%20Web&url=https://www.inithtml.com/`}  target="_blank"  className='icon twitter'>
                      <FaTwitter />
                    </a>

                    <a href='https://www.facebook.com/sharer/sharer.php?u=https://www.inithtml.com/'  target="_blank"  className='icon telegram'>
                      <FaTelegram/>
                    </a>
                  </div>

                  <div className='likes l-6' onClick={() => this.handleLikeItems(items.idItems)}>
                    {isLike === 0 && isActive == false ?
                      <> <FaRegHeart /> <span><SwitchLanguage  id='manageAdmin.items.likes'/></span> </>  
                        : 
                        isActive === true && isLike < 0 ? <> <FaRegHeart /> <span><SwitchLanguage  id='manageAdmin.items.likes'/></span> </>  :
                      <><FaHeart /> <span><SwitchLanguage  id='manageAdmin.items.liked'/></span> </>
                    }
                    
                    <span className='count-likes'>({isLike + countLike})</span>
                  </div>
            ...
            <div className='col l-8'>
              <div className='data-info-items'>
                  ...
                  <>
                    {newPriceUS > 0 && newPrice > 0 ?
                      <div className='list-price-tems-detail'>
                        <span className='discount-items' style={{marginLeft: '0'}}>
                          {languages.EN == language ?  priceUS : <NumberFormat value={price} displayType={'text'} thousandSeparator={true}/>}
                        </span> 
                        <span className='sub-price new'><SwitchLanguage id='manageAdmin.items.subPrice'/></span>
                        
                        <span className='discount-items' style={{marginLeft: '0'}}>{languages.EN == language ? newPriceUS : 
                      ...
                      :
                      <div className='list-price-tems-detail'>
                        <span className='discount-items' style={{marginLeft: '0'}}>{languages.EN == language ? priceUS : 
                          <NumberFormat value={price} displayType={'text'} thousandSeparator={true}/>}</span> 
                        <span className='sub-price new'><SwitchLanguage id='manageAdmin.items.subPrice'/></span>
                      </div>
                    }
                  </>
                :
                  <>
                    {newPriceUS > 0 && newPrice > 0 ?
                      <div className='list-price-tems-detail'>
                        <p>{languages.EN == language ? newPriceUS : <NumberFormat value={newPrice} displayType={'text'} thousandSeparator={true}/>}</p>
                        <span className='sub-price'><SwitchLanguage id='manageAdmin.items.subPrice'/></span>

                        {languages.EN == language ? 
                          <span className='discount-items'>{`${Math.floor(newPriceUS - newPriceUS * Number(discountData.code))}`}</span>  
                        :
                          <span className='discount-items'>{<NumberFormat value={`${Math.floor(newPrice - newPrice * Number(discountData.code))}`} displayType={'text'} thousandSeparator={true}/>}</span>
                        }
                        <span className='sub-price new'><SwitchLanguage id='manageAdmin.items.subPrice'/></span>    
                        
                        <span className='labels-discount'>
                          <SwitchLanguage id='manageAdmin.items.decrease'/> {discountData.valueEn}
                        </span>
                    ...
                    :
                      <div className='list-count-items'>
                        {languages.EN == language ?
                          <div className='list-price-tems-detail'>
                            <p >{languages.EN == language && `${priceUS}`} </p>
                            <span className='sub-price'><SwitchLanguage id='manageAdmin.items.subPrice'/></span>

                            <span className='discount-items'>{`${Math.floor(priceUS - priceUS * Number(discountData.code))}`}</span>  
                            <span className='sub-price new'><SwitchLanguage id='manageAdmin.items.subPrice'/></span>          
                          </div>                      
                        :
                          <div className='list-price-tems-detail'>
                            <p>{languages.EN != language && <NumberFormat value={price} displayType={'text'} thousandSeparator={true}/>}</p>
                            <span className='sub-price'><SwitchLanguage id='manageAdmin.items.subPrice'/></span>

                            <span className='discount-items'>{<NumberFormat value={`${Math.floor(price - price * Number(discountData.code))}`} displayType={'text'} thousandSeparator={true}/>}</span>
                            <span className='sub-price new'><SwitchLanguage id='manageAdmin.items.subPrice'/></span>
              ...
              <div className='detail-price-ship grt-20 '> 
                ...
                <div className='l-10 detail-price-ship  grt-20'>
                  ...
                  {provinceFreeShip.length > 0 || provinceFreeShipCategoryType.length > 0 || provinceFreeShipCategory.length > 0 ?
                    <span className='text_free-ship'><SwitchLanguage id='manageAdmin.items.textFreeShip'/></span> : ''
                  }

                  {provinceFreeShip.length == 0 && provinceFreeShipCategoryType.length == 0 && provinceFreeShipCategory.length == 0 &&
                    <>
                      {allDataPriceShip[0] && allDataPriceShip[0].Province && 
                        <>
                          <span className='text_free-ship l-10'><SwitchLanguage id='manageAdmin.items.nextShip'/></span> 
                          <span className='province-ship-demo '>{languages.EN == language ?  allDataPriceShip[0].Province.valueEn : allDataPriceShip[0].Province.valueVi}</span>
                          <p className='mgl-12 text_free-ship'><SwitchLanguage id='manageAdmin.items.textPriceShip'/></p>
                          <span className='mgl-12 price_ship'>{languages.EN == language ? allDataPriceShip[0].priceShipUS : <NumberFormat value={allDataPriceShip[0].priceShipVN} displayType={'text'} thousandSeparator={true}/>}</span>
                          <span className='sub-price new mgl-4'><SwitchLanguage id='manageAdmin.items.subPrice'/></span>
                        </>
                      }
                  ...
                  <p className='l-12'>
                    {provinceFreeShip.length > 0 &&
                      provinceFreeShip.map(ship => {                
                        return (<span key={ship.Province.keyMap} className='Province-preeShip'>{languages.EN == language && ship.Province ? ship.Province.valueEn : ship.Province.valueVi},</span> )                  
                      })
                    }
                    {provinceFreeShip.length === 0 && provinceFreeShipCategoryType.length > 0 &&
                      provinceFreeShipCategoryType.map(ship => {
                        return (<span key={ship.Province.keyMap} className='Province-preeShip'>{languages.EN == language && ship.Province ? ship.Province.valueEn : ship.Province.valueVi},</span> )                  
                      })
                    }
                    {provinceFreeShipCategoryType.length === 0 && provinceFreeShipCategory.length > 0 &&
              ...
              <div className='detail-price-ship grt-20'> 
                <div className='l-2  grt-12'>
                  <span className='title-truck '><SwitchLanguage id='manageAdmin.items.listColor'/></span>
                </div>
        ...
        <div className='detail-sub-text-items l-12'>
            <div className='col l-12'>
                <p className='title '><SwitchLanguage id='manageAdmin.items.headingSubDetailText'/></p>
            </div>
            ...
            <div className='list-detail'>
                ...
                <span className='col l-10'>
                    {items && items.infoItemsData && items.infoItemsData.production}
                </span>
                ...
                <span className='col l-10'>
                    {items && items.infoItemsData && items.infoItemsData.trademarkData.valueEn}
                </span>
                ...
                <span className='col l-10'>
                    {items && items.infoItemsData && items.infoItemsData.texture}
                </span>
                ...
                <span className='col l-10'>
                    {items && items.infoItemsData && items.infoItemsData.sentFrom}
                </span>
            </div>

            <div className='col l-12'>
                <p className='title '><SwitchLanguage  id='manageAdmin.items.describe-items'/></p>
            </div>
            
            {languages.EN === language ?
                <div className='content-describe-items' dangerouslySetInnerHTML={{__html: items.infoItemsData && items.infoItemsData.describeHtmlEn}}></div>
                :
                <div className='content-describe-items' dangerouslySetInnerHTML={{__html: items.infoItemsData && items.infoItemsData.describeHtmlVi}}></div>
            }
        </div>
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(DetailItems));
