...
class ListItemsSearch extends Component {
  ...
  componentDidUpdate = async (prevProps, prevState) => {
    ...
    if (
      prevProps.isHideFormDataOptionsSearch !==
      this.props.isHideFormDataOptionsSearch
    ) {
      this.setState({
        isShowHideFromListItems: isHideFormDataOptionsSearch,
      });
    ...
    return (
      <>
        {isShowHideFromListItems && (
          <div
            className="wraper-options keyfame"
            ref={this.listResultDataSearch}
            style={{
          ...
          >
            {!isSeeMore && (
              <div className="title-options">
                <span>
                  {" "}
                  <SwitchLanguage id="manageAdmin.items.history" />
                </span>
              </div>
            )}
            ...
            <div
              className="options_items"
              style={{ maxHeight: isSeeMore ? "none" : "350px" }}
            >
              {valueInput !== "" &&
                dataSearchInput &&
                dataSearchInput.length > 0 &&
                dataSearchInput.map((item, index) => {
                  return (
                    <Link
                      ...
                      className="item"
                      to={{
                        pathname: `${path.DETAIL_ITEMS}`,
                        search: `?idItems=${item.idItems}`,
                      }}
                      style={{
                    ...
                    >
                      <div className="wraper-img">
                        <img
                          src={`${process.env.REACT_APP_BACKEND_IMAGES_ITEMS}/${
                      ...
                      <div className="detail">
                        <p className="name-items">
                          {languages.EN === language ? item.nameEn : item.name}
                        </p>
            ...
            {numberItems > limitShowItems && (
              <div className="options-end">
                <Link
                  onClick={() =>
                    setTimeout(() => {
                      this.props.handleShowHideListDataOptions();
                    }, 300)
                  }
                  to={{
                    pathname: `${path.SEARCH_ITEMS}`,
                    search: `?idShop=${dataOptionsSearch[0].idShop}&category=${
                      dataOptionsSearch[0].category
                    }&type=${
                      dataOptionsSearch[0].type
                    }&language=${language}&value=${valueInput}&limit=${
                      ITEMS.SEE_MORE_SHOW_ITEMS_SEARCH
                    }&page=${1}`,
                  }}
                >
