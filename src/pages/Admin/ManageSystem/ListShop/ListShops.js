      ...
      <>
        ...
        <div className="col l-12">
          <p className={cx("heading-manage-user")}>
            <SwitchLanguage id="manageAdmin.form.listSeller" />
          </p>
        </div>
            ...
            <table id={cx("customers")}>
              <tbody>
                <tr>
                  ...
                  <th>Email</th>
                  <th>
                    <SwitchLanguage id="manageAdmin.form.manager" />
                  </th>
                  <th>
                ...
                {listAllShops &&
                  ...
                  listAllShops.map((shop, index) => {
                    return (
                      <tr key={shop.id}>
                        <td>{index + 1}</td>
                        <td>
                          {shop.emailShop && shop.emailShop ? (
                            shop.emailShop
                          ) : (
                        ...
                        <td>
                          {shop.FullName && shop.FullName.firstName ? (
                            language === languages.VI ? (
                              `${shop.FullName.firstName} ${shop.FullName.lastName}`
                            ) : (
                              `${shop.FullName.lastName} ${shop.FullName.firstName}`
                            )
                          ) : (
                            <SwitchLanguage id="manageAdmin.form.emptyData" />
                          )}
                        ...
                        <td>
                          {" "}
                          <Link className={cx("detail-shop")} to="/">
                            <SwitchLanguage id="manageAdmin.form.viewDetail" />
                          </Link>
                        </td>
                        <td>
                          <FontAwesomeIcon
                            onClick={() => this.handleEditUser(shop)}
                            className={cx("icon-tabel")}
                            icon={faUserPen}
                          />
                          <FontAwesomeIcon
                            onClick={() => this.handleDelete(shop)}
                            className={cx("icon-tabel")}
                            icon={faTrashCan}
                          />
                        </td>
