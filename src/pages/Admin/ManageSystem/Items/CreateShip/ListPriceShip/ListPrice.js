...
class ListPrice extends Component {
    ...
    handleSetlistDataSelect = async(action) => {
        ...
        let newListShop = generalHandling.handlConvertObject(allShops, 'LIST_SHOP',language)
        let newAllcategory = generalHandling.handlConvertObject(allcategory, 'LIST_CATEGORY',language)
        ...
        if(action === 'LANGUAGE' ){
            if(optionsCategory !== null){
                newDataCategoryType = generalHandling.handlConvertObject(await this.props.getAllCodeInToItems(optionsCategory.value), 'LIST_CATEGORY',this.props.language)
                newOptionsCategory = newAllcategory.filter(item => item.value === optionsCategory.value)
            }
            if(optionsCategoryType !== null){
                newOptionsCategoryType = newDataCategoryType.filter(item => item.value === optionsCategoryType.value)
            }
        }
    ...
    handlChangeSlelect = async (valueOptions, name) => {
        ...
        if(name.name === 'forItemCategory') {
            ...
            let dataPriceShip = []
            let resData = await adminService.getPriceShip({idShop: optionsIdshop.value , itemsId: 'EMPTY', category: valueOptions.value, categoryType: 'EMPTY'})
        ...
        if(name.name === 'forItemCategoryType') {   
            ...
            let dataPriceShip = []
            let resData = await adminService.getPriceShip({idShop: optionsIdshop.value , itemsId: 'EMPTY', category: optionsCategory.value, categoryType: valueOptions.value})
            if(resData && resData.data && resData.data.data){
                dataPriceShip = resData.data.data
            }
    ...
    handleGetDataComponentSearch = async(items,typeAction) => {
        ...
        if(idItem !== ''){
            let resData = await adminService.getPriceShip({idShop: optionsIdshop.value , itemsId: idItem, category: 'EMPTY', categoryType: 'EMPTY'})
            if(resData && resData.data && resData.data.data){
                dataPriceShip = resData.data.data
            }
        }
    ...
    changePriceShip = async(dataPrice, actions) => {
        ...
        if(actions === 'DELETE'){
            ...
            if(resData && resData.data && resData.data.errCode === 0){
                ...
                if(optionsCategory !== null && optionsCategoryType !== null){
                    this.handlChangeSlelect(optionsCategoryType, {name: 'forItemCategoryType'})
                }
    ...
    let {listAllShops,listAllCategory,listAllCategoryType ,optionsIdshop,optionsCategory,optionsCategoryType,resListDataPriceShip, valueInputSearch} = this.state
            ...
            {resListDataPriceShip && resListDataPriceShip.length > 0 && 
                <div className='detail-price'>
                    ...
                    {resListDataPriceShip[0].Store && 
                        <div className='store'>
                            <div className='border_img'>
                                <img className='avata_shop' src={`${process.env.REACT_APP_BACKEND_IMAGES_AVT_SHOP}/${resListDataPriceShip[0].Store.avata}`} alt='' />
                            </div>

                            <span className='name-shop'>{resListDataPriceShip[0].Store.nameShop}</span>
                        </div>
                    }

                    {isYesModal &&
                        <p className='title'><SwitchLanguage id='manageAdmin.modal.exist' /></p> 
                    }

                    <div className='margin-text'>
                   
                        {/* Category */}
                        {resListDataPriceShip[0].Category !== 'EMPTY' && resListDataPriceShip[0].categoryType === 'EMPTY' && resListDataPriceShip[0].itemsId === 'EMPTY' &&
                            <div className='category'>
                                <SwitchLanguage id='manageAdmin.items.category' /> 
                                <span className='text-note'>{languages.EN === language ? resListDataPriceShip[0].Category.valueEn : resListDataPriceShip[0].Category.valueVi}</span>
                            </div>
                        }
                        

                        {/* Category type */}
                        {resListDataPriceShip[0].categoryType !== 'EMPTY' &&  resListDataPriceShip[0].itemsId === 'EMPTY' &&
                            <div  className='category'>
                                <SwitchLanguage id='manageAdmin.items.addDiscountSub' /> 
                                <p className='text-note type'>{languages.EN === language ? resListDataPriceShip[0].Type.valueEn : resListDataPriceShip[0].Type.valueVi}</p>
                            </div>
                        }

                        
                        {/* Items one */}
                        {resListDataPriceShip[0].itemsId !== 'EMPTY' && resListDataPriceShip[0].categoryType === 'EMPTY' && resListDataPriceShip[0].category === 'EMPTY' && resListDataPriceShip[0].Item &&  resListDataPriceShip[0].Item.dataImgItems &&
                            <div className='category'>
                                <SwitchLanguage id='manageAdmin.items.itemsDiscount' />
                            
                                <div className='img-item'> 
                                    <img className='avata_shop' src={`${process.env.REACT_APP_BACKEND_IMAGES_ITEMS}/${resListDataPriceShip[0].Item.dataImgItems.image}`} alt='' />
                                </div>


                                <div >
                                    <p className='text-note type' style={{color: '#c62929'}}>
                                        {languages.VI === language ?  resListDataPriceShip[0].Item.name.slice(0, 60)+ '...' : resListDataPriceShip[0].Item.nameEn.slice(0, 60)+'...'}
                                    </p>

                                    <p className='code-item'>
                                        <SwitchLanguage id='manageAdmin.items.code' />
                                        <span className='text-note'>{resListDataPriceShip[0].Item.idItems}</span>
                                    </p>
                                </div>
                        ...
                        <p style={{margin: '16px 0'}}><SwitchLanguage id='manageAdmin.form.Province' /> {' : '}</p>
                            ...
                            {resListDataPriceShip.map(item => {
                                return (
                                    <div key={item.id}>
                                        {item.Province && 
                                            <span>
                                                {languages.EN === language ? item.Province.valueEn : item.Province.valueVi}{' : '} 
                                                <SwitchLanguage id='manageAdmin.items.price' />
                                                <span className='text-note'>
                                                    {languages.EN === language ? item.priceShipUS : <NumberFormat value={item.priceShipVN} displayType={'text'} thousandSeparator={true}/>} 
                    ...
                    <div className='list-btn-price'>
                        ...
                        <span onClick={(e) => isYesModal ? handleUpdate(e,'SUBMIT', 'UPDATE') : this.changePriceShip(resListDataPriceShip,'EDIT')}>
                            <Button type='edit-form-data' content={isYesModal ? <SwitchLanguage id='manageAdmin.button.update'/> : <SwitchLanguage id='manageAdmin.button.editNow'/>} 
                                width='100px' border='4px' margin='20px 0 0 0' height='30px'
                            /> 
                        </span>

                        <span onClick={() => this.changePriceShip(resListDataPriceShip[0],'DELETE')}>
                            <Button type='submit-form-data' content={<SwitchLanguage id='manageAdmin.button.delete'/>} 
                                color='var(--color-BTN-manage)' width='100px' border='4px' margin='20px 0 0 0' height='30px'
                            /> 
                        </span>
                    </div>
