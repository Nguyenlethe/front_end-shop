...
function searchItemsNameNav(data) {
  return axios.get(
    `/admin/search-name-items?idShop=${data.idShop}&category=${data.category}&type=${data.type}&language=${data.language}&value=${data.value}&limit=${data.limit}&page=${data.page}`
  );
}
...
function getLikeOrFollowItemsShop(data) {
  return axios.get(
    `/app/get-data-tabel-followLikes?&idUser=${data.idUser}&type=${data.type}&idItems=${data.idItems}&idShop=${data.idShop}`
  );
}
