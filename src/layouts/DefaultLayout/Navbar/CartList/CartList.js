          ...
          <div className="list-items-cart">
            {litsItemsCart.length > 0 && islogin == "true" && (
              <span className="title-cart">
                <SwitchLanguage id="manageAdmin.items.titleCart" />
              </span>
            )}
                    ...
                    return (
                      ...
                      >
                        <div className="wrapper-img-items-cart">
                          <img
                            src={`${process.env.REACT_APP_BACKEND_IMAGES_ITEMS}/${item.Item.dataImgItems.image}`}
                            alt="Anh items"
                          />
                        </div>

                        <div className="detail-info-items">
                          <p className="name-items-cart">
                        ...
                        <div className="list-price-items-cart">
                          <p className="price-items-cart">
                            {languages.EN == language ? (
                              item.Item.newPriceUS > 0 ? (
                                item.Item.newPriceUS
                              ) : (
                                item.Item.priceUS
                              )
                            ) : item.Item.newPrice > 0 ? (
                              <NumberFormat
                                value={item.Item.newPrice}
                                displayType={"text"}
            ...
            {litsItemsCart.length > 0 && islogin == "true" && (
              <button className="btn-next-to-cart">
                {" "}
                <Link to={path.DETAIL_LIST_CART}>Xem Giỏ Hàng</Link>
              </button>
            )}
          </div>
        )}
