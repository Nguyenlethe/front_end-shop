...
class Login extends Component {
  ...
  render() {
    ...
    let { isShowPass, isModal, errMessage, isFocusAcount, isFocusPassword } =
      this.state;
    let { account, password } = this.state.form;
    ...
    return (
      <>
        <span>
          <LoadScroll data={dataUser.isLogin} isShow={isModal} />
        </span>
        ...
        <form className={cx("form-login")}>
          ...
          <div className={cx("form-input")}>
            <label>
              <SwitchLanguage id="manageAdmin.form.account" />
            </label>
            <div
              className={cx("wrap-input")}
              style={{ boxShadow: isFocusAcount ? "1px 1px 10px #046ba6" : "" }}
            >
              ...
              <input
                ...
                autoComplete="off"
                placeholder={
                  language === languages.EN ? "Enter account" : "Nhập tài khoản"
                }
                onFocus={() => this.setState({ isFocusAcount: true })}
                onChange={(e) =>
                  this.handleOnchangeInput(e.target.value, e.target.name)
                }
                onBlur={() => this.setState({ isFocusAcount: false })}
              />
            </div>
            ...
            <div
              className={cx("wrap-input")}
              style={{
                boxShadow: isFocusPassword ? "1px 1px 10px #046ba6" : "",
              }}
            >
              <FontAwesomeIcon
              ...
              <input
                ...
                onFocus={() => this.setState({ isFocusPassword: true })}
                value={password}
                autoComplete="on"
                onBlur={() => this.setState({ isFocusPassword: false })}
                onChange={(e) =>
                  this.handleOnchangeInput(e.target.value, e.target.name)
                }
              />
            ...
            <span className={cx("error-input-login")}>
              ...
              {errMessage.password !== ""
                ? language === languages.EN
                  ? errMessage.password.valueEn
                  : errMessage.password.valueVi
                : ""}
            </span>
            ...
            >
              <Button
                ...
                opacity={
                  account.trim() !== "" && password.trim() !== "" ? "1" : ".6"
                }
