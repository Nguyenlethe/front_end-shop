      ...
      <>
        <div className="list-number-next-page ">
          <Link
            ...
            ref={this.LinkPage}
            to={{
              search: `?idShop=${dataSearchItemsParams.idShop}&category=${
                dataSearchItemsParams.category
              }&type=${
                dataSearchItemsParams.type
              }&language=${language}&value=${value}&limit=${limitSeeMore}&page=${
                numberPageActive - 1 < 1
                  ? arrayNumberPage.length
                  : numberPageActive - 1
              }`,
            }}
          >
          ...
          <div className="border-list-number-page">
            <ul
              className="list-number-page"
              style={{
                width:
                  numberPageActive < 3
                    ? "160px"
                    : `${(numberPageActive + 1) * 40}px`,
              }}
            >
                    ...
                    return (
                      ...
                      >
                        <Link
                          ...
                          to={{
                            search: `?idShop=${dataSearchItemsParams.idShop}&category=${dataSearchItemsParams.category}&type=${dataSearchItemsParams.type}&language=${language}&value=${value}&limit=${limitSeeMore}&page=${page}`,
                          }}
                        >
                          {page}
                        </Link>
          ...
          {arrayNumberPage.length > ITEMS.LIMIT_SHOW_SEEMORE_CLICK_END && (
            <>
              ...
              <span
                className="endPage"
                onClick={() => this.handleGetOrSetItems("END_PAGE")}
              >
                <SwitchLanguage id="manageAdmin.items.endPage" />
              </span>
            </>
          )}
          ...
          <Link
            ...
            to={{
              search: `?idShop=${dataSearchItemsParams.idShop}&category=${
                ...
                numberPageActive + 1 > arrayNumberPage.length
                  ? 1
                  : numberPageActive + 1
              }`,
            }}
          >
            <FontAwesomeIcon className="icon-right" icon={faAngleRight} />
          </Link>
        </div>
      </>
    );
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(NumberPage)
);
